...
import { ConfiguredMachine, defineMachine, MachineConfig } from "../builder";
...
// function mapArrayToObject(list:

function mapRecipesByInput(recipes: Recipe[]): Record<ResourceType, Recipe> {
	return Object.fromEntries(
		recipes.map(recipe => [recipe.input.resource, recipe])
	) as Record<ResourceType, Recipe>;
}

const recipesByInput = mapRecipesByInput(recipes);

// const recipesByInput = arr(recipes).mapToObject(
// 	x => x.input.resource,
// 	x => x
// );


function getConsumption(machine: ArcFurnace) {
	return (
		recipesByInput[machine.inputResource || "none"].input.amount *
    (machine.catalystActive ? 1.6 : 1) *
    machine.upgrades.velocity.effect
	);
}

function getEnergyUsage(machine: any) {
	return (
		recipesByInput[machine.inputResource || "none"].energyUsage *
    Math.sqrt(machine.upgrades.velocity.effect)
	);
}

function getProduction(machine: any) {
...
// interface ArcFurnace extends MachineConfig<"velocity" | "cat"> {
// 	catalystActive: boolean
// 	inputResource: ResourceType
// }


const arcFurnace = defineMachine({
	name: "arcFurnace",
	inputs: [
		{
			accepts: () =>
				recipes
					.filter(x => (x.isUnlocked ? run(x.isUnlocked) : true))
	...
	upgrades: {
		...
		velocity: {
			...
			title: "Plater",
			description:
        "Increase operation speed but only increases Energy usage at sqrt the rate",
			effect: count => Math.pow(1.3, count),
		},
	},
	customLoop(diff) {
		this.inputResource = this.inputItem(0)
			? this.inputItem(0).resource
			: "none";
...
// type BarcFurnace = (typeof arcFurnace)["new"]
